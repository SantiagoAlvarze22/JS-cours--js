...
//1
const createRecommendFoodPortion = dogs => {
  dogs.forEach(dog => {
    dog.recommendedFood = Math.trunc(dog.weight ** 0.75 * 28);
    console.log(dog);
  });
};
createRecommendFoodPortion(dogs);

const howMuchEatMydogByName = function (dog) {
  let text =
  ...
  console.log(
    `${dog.owners.length > 1 ? dog.owners.join(' & ') : dog.owners} ${text}`
  );
};

// console.log(dogs);
//2
const objectSelectByName = function (name) {
  const objectInteresed = dogs.find(dog => dog.owners.includes(name));
  howMuchEatMydogByName(objectInteresed);
};

objectSelectByName('Matilda');
...
const ownersEatTooMuch = dogs
  ...
  .flatMap(dog =>
    console.log(
      `${
        dog.owners.length > 1 ? dog.owners.join(' & ') : dog.owners
      } Dog is eating too much `
    )
  );
...
const ownersEatTooLittle = dogs
  ...
  .flatMap(dog => console.log(`${dog.owners} Dog is eating too little `));
...
//7
const arrayOfDogsEatingWell = dogs.filter(dogEatingOkay);
console.log(arrayOfDogsEatingWell);

//8
//a and b r objects, for that I go through their properties
